...
<!-- 새 항목은 아래에 추가한다 -->
## 9. Suspense 안의 에러가 페이지 전체를 날린다

**세션:** S30
**대상:** `app/stream/error-in-suspense/`

### 관찰

```
0초   폴백 두 개 표시
1초   Boom 이 에러  →  🅱️ 같은 폴더 error.tsx 가 잡음
2초   "✅ 정상 — 2초"  →  나오지 않는다
```
...
```
error.tsx                       ← 대체 범위는 자기 children 전체
  Suspense(정상)  Suspense(Boom)
    page 내용
```

**`error.tsx` 가 `Suspense` 보다 바깥이므로 대체 범위도 Suspense 바깥이다.**
Suspense 단위로 부분 대체되지 않는다.
...
├── 사용자 정보   정상
├── 매출 차트     정상
└── 추천 위젯     외부 API 실패 💥   →  페이지 전체가 error UI
```

위젯 하나 때문에 멀쩡한 나머지까지 못 본다.
부분 격리는 더 안쪽 경계가 필요하다 → `catchError` (S31).

### 곁들여 — curl 로는 안 보인다

서버 응답 HTML 에는 폴백만 있고 error UI 는 없다.
`error.tsx` 는 클라이언트 컴포넌트이고, 에러 경계는 **브라우저에서** 동작하기 때문이다.
서버는 "여기서 에러가 났다"는 신호만 스트림에 실어 보낸다.

> 2일차 오답노트 그대로다 — **"이 현상은 어느 층인가"를 먼저 정하고 도구를 고를 것.**
