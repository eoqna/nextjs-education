...
새 세션을 시작할 때 이 파일을 먼저 읽는다. 마지막 갱신: 2026-09-08 (2)
...
## 🔴 먼저 닫을 것 — 경계 규칙 (같은 규칙 5회 연속 오답)

**설명으로는 다섯 번 안 붙었다. 다음은 실습이다.**

```
9/3  16일차       "Suspense 안쪽만 대체된다"
9/4  18일차       "사용한 함수(error.tsx / catchError)가 결정한다"
9/7  19일차 Q3    "제목만 남는다" + "2초 뒤 정상이 뜬다"  (한 답 안에서 모순)
9/8  20일차 Q3    경계 2개 중 바깥이 잡는다              (실제는 안쪽)
9/8  20일차 확인   Suspense 로 감싼 부분만 대체된다        (실제는 바깥 전체 · 16일차와 동일)
```

일관된 규칙을 적용하는 게 아니라 **그림을 보고 매번 그럴듯한 쪽을 고르고 있다.**
Q3 은 "바깥"(실제 안쪽), 확인 문제는 "Suspense 단위"(실제 바깥) — 방향이 매번 다르다.

규칙은 두 줄이다.

```
① 에러는 가장 가까운 상위 "에러 경계" 까지 올라간다.
   Suspense 는 에러 경계가 아니다 — promise 를 받지 에러를 받지 않는다.
   에러는 Suspense 를 그대로 통과한다.

② 그 경계가 감싼 것 전체가 대체된다. 에러가 난 위치는 범위와 무관하다.
```

**S32 졸업시험 요구사항 5번이 스트리밍이다.** 이 상태로 들어가면 시험 중에 막힌다.
...
**웜업을 먼저 한다 — `notes/quiz-pending.md` 를 편다.** S30·S31 재출제분 5문제가
대기 중이다(2회차). 1회차(9/7)는 Q1✗ Q2✗ Q3✗ Q4⅓ Q5(결론만) 로 무너졌다 —
오답노트 원문 그대로 물으면 맞히는데 배치를 바꾸면 못 꺼낸다(패턴 ④).
**2회차도 무너지면 S32 에 들어가지 말고 S30·S31 을 다시 실습한다.**
...
```
S4 / 1단계 졸업시험 / S18 / S20 / S27 / S30 / S30·S31 퀴즈 Q3
```
...
**④ 문제 형태가 바뀌면 못 꺼낸다 — 웜업 정답률의 착시**

```
오답노트 원문 그대로 출제   4/5 오답 → 5/5 정답 (회복으로 보였다)
같은 내용, 배치를 바꿔 출제  1/5 수준 (19일차)
```

3일차에 본인이 적었다 — *"지식이 없는 게 아니라 문제 형태가 바뀌면 꺼내지 못한다."*
**웜업은 반드시 형태를 바꿔 낸다.** 원문 그대로 내면 익숙함만 측정하게 된다.

**⑤ 코드의 배치를 확인하지 않고 익숙한 모양으로 읽는다 — 3회**

```
19일차 Q3   "catchError 가 들어있다는 사실을 간과하고 답을 작성했다"
19일차 Q5   "save() 와 Page() 가 다른 파일에 있다고 생각하고 풀어버렸다"
20일차 Q3   "문제를 잘못 읽었다"  ← 확인해보니 오독이 아니라 모델 오류였다
```

패턴 ④ 의 실제 정체일 수 있다. 형태가 바뀌어서 못 꺼내는 게 아니라
**바뀐 것을 확인하지 않고 익숙한 쪽으로 읽는 것**이다.
다만 20일차 사례처럼 "잘못 읽었다"가 모델 오류를 덮는 설명으로 쓰일 수 있으니,
그 설명이 나오면 변형 문제로 한 번 더 확인한다.
...
notes/wrong-answers.md         오답 31건 (#서버실행 18 / #프리렌더 8 / #rsc경계 4 / #관찰방법 4 …)
