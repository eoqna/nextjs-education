...
- **문서는 로컬 사본을 최우선으로 본다.** `create-next-app`으로 만든 앱 안에 설치 버전과 정확히 일치하는 공식 문서 444개가 들어 있다 — `node_modules/next/dist/docs/`. 아래 세션들의 📖 링크는 웹 주소지만, **같은 문서가 로컬에도 있으므로 검색이 빠르다.**
  ```bash
  # 예: 'use cache' 가 나오는 문서 찾기
  grep -rl "use cache" labs/01-rsc/node_modules/next/dist/docs/01-app/ | head
  ```
  웹 문서는 로컬에 없는 내용을 볼 때만 쓴다. 블로그·유튜브는 v14/v15 기준이 대부분이라 캐싱·async API·proxy에서 정면 충돌하므로 보지 않는다.
